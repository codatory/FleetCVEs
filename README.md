...
Open <http://127.0.0.1:8080>. On startup, the app syncs the NVD CPE catalog in the background. The **first sync downloads the full catalog** and can take a long time without an NVD API key; the page displays sync progress and catalog totals as records arrive. Later syncs request changed CPEs. Browse with **Previous**/**Next**, or search the catalog and check **Track** for the versions you want polled. Uncheck **In use** to hide a tracked version's findings without losing its triage decisions. Findings are polled in the background and can be marked *mitigated*, *not applicable*, *patched*, or with a custom status.
...
| --- | --- | --- |
| GET | `/api/cpes` | `?search=widget&limit=100&offset=50` |
| GET | `/api/cpes/count` | `?search=widget`; returns `{"total": 42}` |
